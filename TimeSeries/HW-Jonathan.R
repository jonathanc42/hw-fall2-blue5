...
well <- read.xlsx("Well Data/G-2866_T.xlsx",sheet=3)

#Create Date Time Object

well$datetime<-convertToDateTime(well$date)
well$time1<-convertToDateTime(well$time)
well$date1<-convertToDate(well$date)

well$datetime <- paste(well$Date, well$Time)
well$datetime <- as.POSIXct(well$datetime, format="%Y-%m-%d %H:%M")

format(well$time1,"%H:%M")
