...
# tide <- mutate(tide, datetime=date(Date))  # adds date to datetime
# hour(tide$datetime) <- hour(tide$Time) # Adds hour to datetime. Removes minutes from all hours
# tide$datetime <- as.POSIXct(tide$datetime)
...
#Merge Left Outer join of time sequence on well data
df_F_45 <- time_seq %>%
  left_join(F_45__well_agg, by='datetime') %>%
